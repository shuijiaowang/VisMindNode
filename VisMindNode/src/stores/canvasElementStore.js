...
export const useCanvasElementStore = defineStore('canvasElement', () => {
    ...
    const rootIds = reactive([]) // 顶级元素ID（parentId为null），有序的。
    const selectedElementIds = reactive(new Set()) // 选中的元素ID

    // 组件类型配置（核心：统一管理所有元素类型,默认配置等）
    ...
    const createElement = (options) => {
        ...
        const element = {
            ...
            parentId: options.parentId || directoryStore.selectedDirTitleId, //我想要这个是值，而不是响应式
            children: [],
